...
auto generateTIKZ(fmc::search_scheme::Search _s, int newLen, bool displayAlphabet, double fontSize, bool zeroIndex) -> std::string {
    assert(isValid(_s));
...
/*        if (level == maxLevel) {
    ...
    auto counts = fmc::search_scheme::expandCount(_s.pi.size(), s.pi.size());
    int  accum  = 0;
    out += fmt::format("\\node[] (sl0) at (-1, 0) {{}};\n");
